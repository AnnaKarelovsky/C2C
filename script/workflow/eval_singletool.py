"""
Simplified singletool evaluation on BrowseComp.

Runs singletool mode (run_with_tools) on BrowseComp questions with
multiprocessing support, collects full trajectories, and optionally
runs LLM judge.

Usage:
    # Quick test
    python script/workflow/eval_singletool.py --limit 2 --num-workers 1

    # Full run with custom model
    python script/workflow/eval_singletool.py --limit 100 --num-workers 4 \
        --model-provider fireworks \
        --model-type accounts/fireworks/models/gpt-oss-120b

    # Judge existing results
    python script/workflow/eval_singletool.py --judge-only \
        --output local/evaluation/singletool/browsecomp.jsonl
...
from rosetta.workflow.browse_searcher import (
    configure_search,
    get_document,
    search,
)
...
def load_browsecomp(path: str, limit: list[int]) -> list[dict]:
    """Load BrowseComp QA pairs from JSONL file.

    Args:
        limit: [N] to load first N items, or [start, end] to slice.
               end can be -1 to mean "until the end".

    Returns list of dicts with keys: id, question, answer, _idx.
    """
    data_path = Path(path)
...
def worker(
    ...
    # Configure BrowseComp search in this process
    configure_search(
...
def run_judge(output_path: Path, args: argparse.Namespace) -> None:
    ...
    if category_counts:
        ...
        for cat, examples in sorted(category_counts.items(), key=lambda x: -len(x[1])):
            pct = len(examples) / total_incorrect * 100 if total_incorrect else 0
            table.add_row(cat, f"{len(examples)}/{total_incorrect}", f"{pct:.1f}%")
        console.print(table)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Singletool evaluation on BrowseComp")

    # Data
    parser.add_argument("--data", default="local/data/BrowseCompPlus/data/browsecomp_qa_pairs.jsonl")
    parser.add_argument("--output", default="local/trajectory/singletool/browsecomp.jsonl")
    ...
    # ------- Load data -------
    examples = load_browsecomp(args.data, args.limit)
